...
public class QHCustomer extends BidCustomer {

    private HostnameVerifier hv;

    public QHCustomer(RedisQueue queue) throws Exception {
        super(queue);
    ...
    public void process(String jsonStr) {
        try {
            ...
            if (json.getUrl().contains(".pdf") || json.getUrl().contains(".doc")) {
                Save(json.getTitle(), json.getPubtime(), "", json.getUrl(), "青海省公共资源交易网");
            } else {
                conn.method(Connection.Method.GET).timeout(10000);
                Response response = conn.execute().charset("utf-8");
                Document doc = Jsoup.parse(response.body());
                String info = doc.select("#page > div.ewb-main.ewb-mbg > div > div.ewb-list-bd > div > div.xiangxiyekuang").outerHtml();
                Save(json.getTitle(), json.getPubtime(), info, json.getUrl(), "青海省公共资源交易网");
            }
        } catch (Exception e) {
    ...
    public static void main(String args[]) {
        try {
            ...
            QHCustomer b = new QHCustomer(queue);
