...
public class LNTask {
    ...
    private static final Logger logger = LoggerFactory.getLogger(LNTask.class);

    public static void task() {
        logger.info("辽宁省公共资源交易网数据爬取任务开始");
        RedisQueue queue = new RedisQueue("lnggzy_queue");
        ExecutorService pexecutorService = Executors.newFixedThreadPool(1); // 生产者线程池
        pexecutorService.execute(new LNProduct(queue));
        pexecutorService.shutdown();
        int customer = 2;
        ExecutorService cexecutorService = Executors.newFixedThreadPool(customer); // 消费者线程池
        for (int i = 1; i < customer; i++) {
            try {
                Thread t = new Thread(new LNCustomer(queue));
                t.setName("LNCustomer_" + String.valueOf(i + 1));
                cexecutorService.execute(t);
            } catch (Exception e) {
                logger.error("LNCustomer_" + String.valueOf(i + 1) + "线程启动错误，错误信息:" + ExceptionUtil.getStackTrace(e));
            }
        }
