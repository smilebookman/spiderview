...
public class NMGCustomer extends BidCustomer {

    private HostnameVerifier hv;

    public NMGCustomer(RedisQueue queue) throws Exception {
        super(queue);
    ...
    public void process(String jsonStr) {
        try {
            ...
            if (json.getUrl().contains(".pdf") || json.getUrl().contains(".doc")) {
                Save(json.getTitle(), json.getPubtime(), "", json.getUrl(), "内蒙古公共资源交易网");
            } else {
                conn.method(Connection.Method.GET).timeout(10000);
                Response response = conn.execute().charset("utf-8");
                Document doc = Jsoup.parse(response.body());
                String info = doc.select("body > div.container.layout.clearfloat > div.content > div.detail_contect").outerHtml();
                Save(json.getTitle(), json.getPubtime(), info, json.getUrl(), "内蒙古公共资源交易网");
            }
        } catch (Exception e) {
    ...
    public static void main(String args[]) {
        try {
            ...
            NMGCustomer b = new NMGCustomer(queue);
