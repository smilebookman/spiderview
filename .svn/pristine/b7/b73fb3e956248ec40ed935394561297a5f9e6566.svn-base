 ...
 * @author lh
 */
public class HeBeiTask {
     private static final Logger logger = LoggerFactory.getLogger(HeBeiTask.class);

    public static void task() {
        logger.info("河北省公共资源交易中心数据爬取任务开始");
        RedisQueue queue = new RedisQueue("hebeiggzy_queue");
        ExecutorService pexecutorService = Executors.newFixedThreadPool(1); // 生产者线程池
        pexecutorService.execute(new HeBeiProduct(queue));
        pexecutorService.shutdown();
        int customer = 2;
        ExecutorService cexecutorService = Executors.newFixedThreadPool(customer); // 消费者线程池
        for (int i = 1; i < customer; i++) {
            try {
                Thread t = new Thread(new HeBeiCustomer(queue));
                t.setName("HeBeiCustomer_" + String.valueOf(i + 1));
                cexecutorService.execute(t);
            } catch (Exception e) {
                logger.error("HeBeiCustomer_" + String.valueOf(i + 1) + "线程启动错误，错误信息:" + ExceptionUtil.getStackTrace(e));
            }
        }
