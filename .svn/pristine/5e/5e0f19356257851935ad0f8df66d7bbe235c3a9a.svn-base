...
public class JSCustomer extends BidCustomer {

    private HostnameVerifier hv;

    public JSCustomer(RedisQueue queue) throws Exception {
        super(queue);
    ...
    public void process(String jsonStr) {
        try {
            ...
            if (json.getUrl().contains(".pdf")) {
                Save(json.getTitle(), json.getPubtime(), "", json.getUrl(), "江苏省公共资源交易网");
            } else {
                conn.method(Connection.Method.GET).timeout(10000);
                Response response = conn.execute().charset("utf-8");
                Document doc = Jsoup.parse(response.body());
                String info = doc.select("body > div.ewb-trade-mid > div > div.ewb-page-main.ewb-h543 > div > div.article-info > div.con").outerHtml();
                Save(json.getTitle(), json.getPubtime(), info, json.getUrl(), "江苏省公共资源交易网");
            }
        } catch (Exception e) {
    ...
    public static void main(String args[]) {
        try {
            ...
            JSCustomer b = new JSCustomer(queue);
