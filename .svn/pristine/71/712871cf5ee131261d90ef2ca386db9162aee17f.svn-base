...
public class SXTask {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SXTask.class);

    public static void task() {
        logger.info("陕西省公共资源交易平台数据爬取任务开始");
        RedisQueue queue = new RedisQueue("sxggzy_queue");
        ExecutorService pexecutorService = Executors.newFixedThreadPool(1); // 生产者线程池
        pexecutorService.execute(new SXProduct(queue));
        pexecutorService.shutdown();
        int customer = 2;
        ExecutorService cexecutorService = Executors.newFixedThreadPool(customer); // 消费者线程池
        for (int i = 1; i < customer; i++) {
            try {
                Thread t = new Thread(new SXCustomer(queue));
                t.setName("SXCustomer_" + String.valueOf(i + 1));
                cexecutorService.execute(t);
            } catch (Exception e) {
                logger.error("SXCustomer_" + String.valueOf(i + 1) + "线程启动错误，错误信息:" + ExceptionUtil.getStackTrace(e));
            }
        }
