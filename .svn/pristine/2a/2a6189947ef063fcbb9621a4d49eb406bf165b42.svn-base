 ...
 * @author cxyue
 */
public class GuiZhouCustomer extends BidCustomer {

    public GuiZhouCustomer(RedisQueue queue) throws Exception {
        super(queue);
    }
    ...
    public void process(String jsonStr) {
        try {
            ...
            String info = doc.select("body > div.article_box > div > div.article_wrap > div.detail_box").outerHtml();
            if (info.contains("<img")) {
                info = info.replace(info.valueOf("/u/cms/"), "http://www.gzsggzyjyzx.cn/u/cms/");
            }
            Save(json.getTitle(), json.getPubtime(), info, json.getUrl(), "贵州省公共资源交易中心");
            Thread.sleep(3000);
        } catch (Exception ex) {
            Logger.getLogger(GuiZhouCustomer.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public static void main(String args[]) {
        try {
            ...
            GuiZhouCustomer b = new GuiZhouCustomer(queue);
