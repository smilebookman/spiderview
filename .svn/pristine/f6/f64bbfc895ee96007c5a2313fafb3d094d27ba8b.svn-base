 ...
 */
public class GuangXiTask {

    private static final Logger logger = LoggerFactory.getLogger(GuangXiTask.class);

    public static void task() {
        logger.info("广西壮族自治区公共资源交易中心网数据爬取任务开始");
        RedisQueue queue = new RedisQueue("guangxiggzy_queue");
        ExecutorService pexecutorService = Executors.newFixedThreadPool(1); // 生产者线程池
        pexecutorService.execute(new GuangXiProduct(queue));
        pexecutorService.shutdown();
        int customer = 2;
        ExecutorService cexecutorService = Executors.newFixedThreadPool(customer); // 消费者线程池
        for (int i = 1; i < customer; i++) {
            try {
                Thread t = new Thread(new GuangXiCustomer(queue));
                t.setName("GuangXiCustomer_" + String.valueOf(i + 1));
                cexecutorService.execute(t);
            } catch (Exception e) {
                logger.error("GuangXiCustomer_" + String.valueOf(i + 1) + "线程启动错误，错误信息:" + ExceptionUtil.getStackTrace(e));
            }
        }
