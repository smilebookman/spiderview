 ...
 * @author lh
 */
public class HeiLongJiangTask {
     private static final Logger logger = LoggerFactory.getLogger(HeiLongJiangTask.class);

    public static void task() {
        logger.info("黑龙江省公共资源交易中心数据爬取任务开始");
        ...
        pexecutorService.execute(new HeiLongJiangProduct(queue));
        pexecutorService.shutdown();
        int customer = 2;
        ExecutorService cexecutorService = Executors.newFixedThreadPool(customer); // 消费者线程池
        for (int i = 1; i < customer; i++) {
            try {
                Thread t = new Thread(new HeiLongJiangCustomer(queue));
                t.setName("HeiLongJiangCustomer_" + String.valueOf(i + 1));
                cexecutorService.execute(t);
            } catch (Exception e) {
                logger.error("HeiLongJiangCustomer_" + String.valueOf(i + 1) + "线程启动错误，错误信息:" + ExceptionUtil.getStackTrace(e));
            }
        }
