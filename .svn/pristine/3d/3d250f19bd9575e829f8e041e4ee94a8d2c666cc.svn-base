...
public class NXCustomer extends BidCustomer {

    private HostnameVerifier hv;

    public NXCustomer(RedisQueue queue) throws Exception {
        super(queue);
    ...
    public void process(String jsonStr) {
        try {
            ...
            if (json.getUrl().contains(".pdf") || json.getUrl().contains(".doc")) {
                Save(json.getTitle(), json.getPubtime(), "", json.getUrl(), "宁夏公共资源交易网");
            } else {
                ...
                if (info.equals("")) {
                    info = doc.select("body > div:nth-child(3) > div.list-con.file_box > div.info_box").outerHtml();
                }
                if (info.equals("")) {
                    info = doc.select("#bodyContent > tbody > tr:nth-child(6) > td").outerHtml();
                }
                Save(json.getTitle(), json.getPubtime(), info, json.getUrl(), "宁夏公共资源交易网");
            }
        } catch (Exception e) {
    ...
    public static void main(String args[]) {
        try {
            ...
            NXCustomer b = new NXCustomer(queue);
